/**
 * @file uart_frame_config.h
 * @brief AGV UART帧协议项目配置
 * @details 用于第三方控制器与AGV底盘MCU之间的UART通信
 *
 * @warning 请勿修改协议字段顺序、字段类型、枚举数值或packed属性
...
#ifndef UART_FRAME_CONFIG_H
#define UART_FRAME_CONFIG_H

#include <stdint.h>
#include <stdbool.h>
#include <math.h>  /* float_t 的定义 */

#if defined(__CC_ARM)
#define UART_FRAME_PACKED_ATTR
#else
#define UART_FRAME_PACKED_ATTR __attribute__((packed))
#endif
...
#define UART_FRAME_DATA_MAX_SIZE 1024
    ...
    /*
     * 协议兼容性提示：本协议在STM32侧假设 sizeof(float_t) == 4。
     * 不要将下方的 float_t 擅自改成 double，也不要调整任意字段顺序。
     */
...
#define IMU_ARRAY_NUM 5
    ...
    typedef struct // size 8
    {
        ...
        struct // size (20 + 4 + 36) * 5 =
        {
            ...
            float_t temperature;    // 温度 (°C, 来自陀螺仪)

            float_t accel_x, accel_y, accel_z;       // 加速度 (m/s²)
            float_t roll_6dof, pitch_6dof, yaw_6dof; // 6轴姿态（陀螺仪+加速度计）
            float_t roll_9dof, pitch_9dof, yaw_9dof; // 9轴姿态（加入磁力计修正）
        } imu[IMU_ARRAY_NUM];
    ...
    } UART_FRAME_PACKED_ATTR sensor_data_t;
    ...
    typedef struct
    {
        ...
        {
            // 角度单位：度；0°为车头正前方。
            // 接口文档约定：顺时针为正；首次上车前请以+5°/-5°低风险实测确认。
            float_t angle_left;
            float_t angle_right;
        ...
        // uint8_t pad[64]; // 填充
    } UART_FRAME_PACKED_ATTR ctrl_frame_t;
    ...
    typedef struct
    {
        ...
        struct
        {
            ...
            float_t kd;
#endif                        // UART_PID_DEBUG
            float_t left_laps;  // 左轮圈数
            float_t right_laps; // 右轮圈数
        } motor_rpm;          // 电机转速
    ...
    } UART_FRAME_PACKED_ATTR status_feedback_t;
